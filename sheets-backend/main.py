...
class ClassRequest(BaseModel):
    ...
    sorting_mode: Optional[str] = None
...
def get_password_hash(password: str) -> str:
    ...
    return f"pbkdf2_sha256${PASSWORD_HASH_ITERATIONS}${salt}${hashed}"


def verify_password(plain_password: str, hashed_password: str) -> bool:
    """Verify password against PBKDF2 hash (with legacy SHA-256 fallback)."""
...
def check_rate_limit(limit_key: str, limit: int, window_seconds: int) -> bool:
    now = time.time()
    window_start = now - window_seconds
    attempts = [ts for ts in rate_limit_store.get(limit_key, []) if ts >= window_start]
    if len(attempts) >= limit:
        rate_limit_store[limit_key] = attempts
        return False
    attempts.append(now)
    rate_limit_store[limit_key] = attempts
    return True
...
def mask_email(email: str) -> str:
    """Mask email for logs to avoid leaking full identifiers."""
    if "@" not in email:
        return "***"
...
def mask_email_for_teacher_response(email: Optional[str]) -> str:
    """Mask student email in teacher-facing API payloads."""
    if not email:
        return ""
...
def send_verification_email(to_email: str, code: str, name: str):
    ...
    try:
        ...
        print(f"✅ Verification email sent to {mask_email(to_email)}")
        return True
...
def send_password_reset_email(to_email: str, code: str, name: str):
    ...
    try:
        ...
        api_response = api_instance.send_transac_email(send_smtp_email)
        print(f"✅ Password reset email sent to {mask_email(to_email)}")
        return True
...
def send_untrusted_device_alert(to_email: str, name: str, device_info: Dict[str, Any]):
    ...
    try:
        ...
        api_response = api_instance.send_transac_email(send_smtp_email)
        print(f"✅ Untrusted device alert sent to {mask_email(to_email)}")
        return True
...
async def signup(request: SignupRequest):
    ...
    try:
        ...
        print(f"✅ TEACHER SIGNUP: {mask_email(request.email)}")
        ...
        return {
            "success": True,
            "message": "Verification code sent to your email" if email_sent else "Unable to send verification email right now. Please try again."
        }
...
@app.post("/auth/verify-email-legacy", response_model=TokenResponse)
async def verify_email_legacy(request: VerifyEmailRequest, response: Response):
    ...
    try:
        ...
        if not db.verify_verification_code(request.email, request.code, stored_data):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def login(request: LoginRequest, response: Response):
    ...
    if is_legacy_password_hash(user.get("password", "")):
        db.update_user(user["id"], password=get_password_hash(request.password))
    
    # ✅ NO DEVICE CHECKING FOR TEACHERS - Direct login
    print(f"✅ TEACHER LOGIN: {mask_email(request.email)} (no device verification)")
...
async def resend_verification(request: ResendVerificationRequest):
    ...
    try:
        ...
        # Generate new code
        code = generate_verification_code()
        print(f"New verification code issued for {mask_email(request.email)}")
        ...
        return {
            "success": True,
            "message": "New verification code sent to your email" if email_sent else "Unable to send verification email right now. Please try again."
        }
    except HTTPException:
        raise
    except Exception as e:
...
async def request_password_reset(request: PasswordResetRequest):
    ...
    if email_sent:
        print(f"✅ Password reset email sent successfully to {mask_email(request.email)}")
    else:
        print(f"❌ Failed to send password reset email to {mask_email(request.email)}")
    return {"success": True, "message": "If account exists, reset code sent"}
...
async def reset_password(request: VerifyResetCodeRequest):
    ...
    if not db.verify_password_reset_code(request.email, request.code, stored_data):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def change_password(request: ChangePasswordRequest, email: str = Depends(verify_token)):
    ...
    if not db.verify_password_reset_code(email, request.code, stored_data):
...
async def logout(response: Response, email: str = Depends(verify_token)):
    """Logout user"""
...
async def student_signup(request: SignupRequest):
    ...
    try:
        ...
        if request.device_id and request.device_info:
            print(f"📱 STUDENT SIGNUP: {mask_email(request.email)}")
            print(f"   Device: {request.device_info.get('name')} (ID: {request.device_id})")
        else:
            print(f"📱 STUDENT SIGNUP: {mask_email(request.email)} (no device info)")
...
async def verify_student_email(request: VerifyEmailRequest, response: Response):
    """
    ...
    try:
        ...
        if stored_data.get("device_id") and stored_data.get("device_info"):
            ...
            print(f"✅ First device auto-trusted for student: {mask_email(request.email)}")
...
async def student_login(request: LoginRequest, response: Response):
    """
    ...
    if is_legacy_password_hash(user.get("password", "")):
        db.update_student(user["id"], {"password": get_password_hash(request.password)})
    
    # 🔐 CHECK DEVICE FINGERPRINT
    if request.device_id and request.device_info:
        if not is_trusted_device(user, request.device_id):
            # NEW DEVICE DETECTED
            print(f"🚨 NEW DEVICE LOGIN ATTEMPT (STUDENT): {mask_email(request.email)}")
            ...
            print(f"✅ STUDENT LOGIN (TRUSTED DEVICE): {mask_email(request.email)}")
            add_trusted_device(user["id"], request.device_info)
    else:
        # No device info provided - block for security
        print(f"⚠️ STUDENT LOGIN BLOCKED (NO DEVICE INFO): {mask_email(request.email)}")
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def delete_student_account(email: str = Depends(verify_token)):
    ...
    try:
        print(f"API: Delete student account request for {mask_email(email)}")
...
def request_device_access(device_request: DeviceRequestCreate, http_request: Request):
    """
    ...
    try:
        print(f"\n{'='*60}")
        print(f"[DEVICE_REQUEST] New request from {mask_email(device_request.email)}")
        print(f"  Reason: {device_request.reason}")
        print(f"{'='*60}")

        client_ip = http_request.client.host if http_request.client else "unknown"
        if not check_rate_limit(f"device_request:ip:{client_ip}", limit=20, window_seconds=60 * 10):
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                detail="Too many requests. Please try again later."
            )
        if not check_rate_limit(f"device_request:email:{device_request.email}", limit=5, window_seconds=60 * 30):
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
        ...
        student = db.get_student_by_email(device_request.email)
        if not student:
            print(f"[DEVICE_REQUEST] ❌ Student not found: {mask_email(device_request.email)}")
            return {
                "success": True,
                "message": "If account exists, device request has been submitted",
                "request_id": None,
        ...
        if any(d.get("id") == device_request.device_id for d in trusted_devices):
        ...
        other_student = db.find_student_by_device(device_request.device_id)
        if other_student and other_student["id"] != student_id:
            print(f"[DEVICE_REQUEST] ❌ Device linked to another student")
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="DEVICE_ALREADY_LINKED|This device is already linked to another account"
            )
        ...
        request_id = db.create_device_request({
            ...
            "student_email": device_request.email,
            "teacher_id": teacher_id,
            "teacher_name": teacher_name,
            "device_id": device_request.device_id,
            "device_info": device_request.device_info,
...
async def get_device_requests(email: str = Depends(verify_token)):
    ...
    try:
        print(f"\n[DEVICE_REQUESTS] Request from: {mask_email(email)}")
        ...
        # Privacy: only return masked student emails to teachers in this listing.
        masked_requests = []
        for req in requests:
            item = dict(req)
            item["student_email"] = mask_email_for_teacher_response(item.get("student_email"))
            masked_requests.append(item)
            ...
            if student:
                ...
                if devices:  # Only include students who have devices
                    student_devices.append({
                        ...
                        "student_email": mask_email_for_teacher_response(student.get("email", "")),
                        "devices": devices
                    })
...
def rotate_qr_code_if_needed(session: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if elapsed_seconds >= (rotation_interval - 0.5):
        ...
        print(f"[ROTATE] ✅ Code rotated (after {elapsed_seconds:.1f}s)")
        return session
    else:
...
async def start_qr_session(request: dict, email: str = Depends(verify_token)):
    ...
    try:
        ...
        print(f"[QR_START] ✅ Session started")
        print(f"[QR_START] ═══════════════════════════════════\n")
...
async def get_qr_session(class_id: str, date: str, email: str = Depends(verify_token)):
    ...
    print(f"[QR_SESSION] Rotation check complete\n")
...
async def scan_qr_code(request: QRScanRequest, email: str = Depends(verify_token)):
    ...
    print(f"\n[QR_SCAN] ═══════════════════════════════════")
    print(f"[QR_SCAN] Request from {mask_email(email)}")
    ...
    try:
        ...
        print(f"[QR_SCAN] ✓ Parsed: date={date}")
