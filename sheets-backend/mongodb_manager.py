...
class MongoDBManager:
    ...
    def __init__(self, mongo_uri: str, db_name: str = "lernova_db"):
        ...
        try:
            ...
            self._otp_hash_pepper = os.getenv("OTP_HASH_PEPPER") or os.getenv("SECRET_KEY", "")
    ...
    def _create_indexes(self):
        ...
        def _ensure_index(collection, keys, *, unique: bool = False, expire_after_seconds: Optional[int] = None):
            ...
            for name, info in existing.items():
                if info.get("key") == desired_key:
                    ...
                    existing_expire = info.get("expireAfterSeconds")
                    if unique and not existing_unique:
                        try:
                            ...
                            return
                    elif expire_after_seconds is not None and existing_expire != expire_after_seconds:
                        try:
                            collection.drop_index(name)
                        except Exception as drop_err:
                            print(f"⚠️ Warning: Could not drop conflicting TTL index {name}: {drop_err}")
                            return
                    else:
                        return
        ...
        _ensure_index(self.verification_codes, [("expires_at_dt", ASCENDING)], unique=False, expire_after_seconds=0)
        _ensure_index(self.password_reset_codes, [("expires_at_dt", ASCENDING)], unique=False, expire_after_seconds=0)

        print("✅ MongoDB indexes ensured")

    def _hash_one_time_code(self, code: str) -> str:
        normalized = str(code).strip()
        if self._otp_hash_pepper:
            return hmac.new(
                self._otp_hash_pepper.encode("utf-8"),
    ...
    def update_class(self, user_id: str, class_id: str, class_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if ("sorting_mode" not in class_data) or (class_data.get("sorting_mode") in (None, "")):
            class_data["sorting_mode"] = existing_class.get(
                "sorting_mode",
                "file_order" if existing_class.get("enrollment_mode") == "import_data" else "ascending"
            )
    ...
    def store_verification_code(self, email: str, code: str, data: Dict[str, Any]) -> None:
        ...
        if isinstance(expires_raw, datetime):
            expires_at_dt = expires_raw
        elif isinstance(expires_raw, str):
            try:
                expires_at_dt = datetime.fromisoformat(expires_raw)
            except ValueError:
                expires_at_dt = None

        verification_data = {
            "email": email,
            "code_hash": self._hash_one_time_code(code),
            "created_at": datetime.utcnow().isoformat(),
    ...
    def verify_verification_code(self, email: str, submitted_code: str, stored_data: Optional[Dict[str, Any]] = None) -> bool:
        record = stored_data or self.get_verification_code(email)
        if not record:
            return False

        submitted_hash = self._hash_one_time_code(submitted_code)
        stored_hash = record.get("code_hash")
        if stored_hash and hmac.compare_digest(stored_hash, submitted_hash):
            return True

        # Backward compatibility for legacy records that still store plaintext `code`.
        legacy_code = record.get("code")
        if isinstance(legacy_code, str) and hmac.compare_digest(legacy_code, str(submitted_code).strip()):
            self.verification_codes.update_one(
                {"email": email},
                {"$set": {"code_hash": submitted_hash}, "$unset": {"code": ""}}
            )
            return True
    ...
    def verify_password_reset_code(self, email: str, submitted_code: str, stored_data: Optional[Dict[str, Any]] = None) -> bool:
        record = stored_data or self.get_password_reset_code(email)
        if not record:
            return False
